...
int compute(XAxiDma *dma, u8 *x_test, int *results);

int main(void)
{
	int Status;
	XAxiDma_Config *Config;
	u8 *x_test = (u8 *) X_TEST_BASE;
	u8 *y_test = (u8 *) Y_TEST_BASE;
	int *results = (int *) RESULTS_BASE;
	...
	for (int i = 0; i < TEST_CASES; i++) {
		...
		for (int j = 0; j < OUTPUT_DIM; j++) {
			if (results[i * OUTPUT_DIM + j] > max) {
				max = results[i * OUTPUT_DIM + j];
				actual = j;
			}

			if (y_test[i * OUTPUT_DIM + j] == 1) {
				expected = j;
			}
		}
...
int compute(XAxiDma *dma, u8 *x_test, int *results)
{
	...
	xil_printf("Starting MNIST test.\n");
	for (int i = 0; i < TEST_CASES; i++) {
		Status = XAxiDma_SimpleTransfer(dma, (UINTPTR) (results + i * OUTPUT_DIM), OUTPUT_DIM * 4, XAXIDMA_DEVICE_TO_DMA);
		if (Status != XST_SUCCESS) {
			xil_printf("Failed to initiate DMA from device.\n");
			return XST_FAILURE;
		}

		Status = XAxiDma_SimpleTransfer(dma, (UINTPTR) (x_test + i * INPUT_DIM), INPUT_DIM, XAXIDMA_DMA_TO_DEVICE);
		if (Status != XST_SUCCESS) {
