...
int compute(XAxiDma *dma, float *x_test, float *results);

int main(void)
{
	int Status;
	XAxiDma_Config *Config;
	float *x_test = (float *) X_TEST_BASE;
	float *results = (float *) RESULTS_BASE;
...
int compute(XAxiDma *dma, float *x_test, float *results)
{
	...
	for (int i = 0; i < TEST_CASES; i++) {
		...
		Status = XAxiDma_SimpleTransfer(dma, (UINTPTR) (x_test + i * INPUT_DIM), INPUT_DIM * 4, XAXIDMA_DMA_TO_DEVICE);
		if (Status != XST_SUCCESS) {
