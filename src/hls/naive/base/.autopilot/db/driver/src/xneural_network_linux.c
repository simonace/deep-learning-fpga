...
typedef struct {
    ...
    XNeural_network_uio_map maps[ MAX_UIO_MAPS ];
} XNeural_network_uio_info;

/***************** Variable Definitions **************************************/
static XNeural_network_uio_info uio_info;
...
static int uio_info_read_name(XNeural_network_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XNeural_network_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XNeural_network_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XNeural_network_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XNeural_network_Initialize(XNeural_network *InstancePtr, const char* InstanceName) {
	XNeural_network_uio_info *InfoPtr = &uio_info;
