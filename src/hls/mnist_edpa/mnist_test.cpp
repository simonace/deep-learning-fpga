...
int main()
{
	...
	for (int i = 0; i < TEST_CASES; i++) {
	    ...
	    packer<data_t, 16, 784>(data_in, sdata_in);
		mnist_edp(sdata_in, sdata_out);
	    unpacker<result_data_t, 16, 10>(sdata_out, result);

		// Write the prediction into the results file as 16-bit unsigned integers
		int answer;
		while (!result.empty()) {
